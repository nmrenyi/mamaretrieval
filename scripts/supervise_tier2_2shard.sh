#!/usr/bin/env bash
# Tier 2 supervisor: 2 shards on H100, each with WORKERS=32.
#
# - Tracks each shard's state independently
# - Reports vLLM load, judge start, periodic progress per shard
# - Auto-resubmits a shard on persistent terminal failure (with 60s
#   re-check to let runai auto-recover transient errors)
# - Exits 0 when BOTH shards Succeed; syncs both outputs and prints
#   the concat command
#
...
JOB0=mamaretrieval-judge-v2-full-h100-shard0
JOB1=mamaretrieval-judge-v2-full-h100-shard1
LOG0="$SCRATCH/logs/vllm_judge_shard0.log"
LOG1="$SCRATCH/logs/vllm_judge_shard1.log"
OUT0="$SCRATCH/data/audit/v2_full_h100_shard0.jsonl"
OUT1="$SCRATCH/data/audit/v2_full_h100_shard1.jsonl"
RAW0="$SCRATCH/data/audit/v2_full_h100_shard0.raw.jsonl"
RAW1="$SCRATCH/data/audit/v2_full_h100_shard1.raw.jsonl"
TARGET_TOTAL=36418
TARGET_PER_SHARD=$((TARGET_TOTAL / 2))   # approximate; queries split by shard

MAX_ATTEMPTS=10

# Per-shard state (suffix 0 / 1)
...
resubmit() {
  ...
  sleep 15
  cd "$LOCAL_REPO"
  # Resubmit BOTH shards via the standard submit script (it submits SHARD_COUNT
  # jobs, so resubmitting only one isn't directly supported). Instead we use
  # the raw runai submit command pattern. But the simplest hack: re-run the
  # full submit; the OTHER shard already exists and will be 'replaced' by the
  # 'runai delete job' step inside the submit script. To avoid disrupting the
  # other shard, we manually re-create only the failed shard using runai
  # describe + submit.
  # Pragmatic approach: just resubmit the entire 2-shard batch. The other
  # shard (which may be Running) will be deleted+recreated, losing its progress.
  # Resume mode (--resume) is on by default, so it picks up from the existing
  # output file. But the vLLM warmup time is lost.
  # Future improvement: per-shard submission.
  ...
  if echo "$out" | grep -q "submitted successfully"; then
    ...
    # Reset state for both shards since both pods were re-issued
    for s in 0 1; do
      eval "state_$s=''"
      eval "load_$s=-1"
      eval "ready_$s=0"
...
while true; do
  for shard in 0 1; do
    ...
    if [ "$state" = "Running" ] && [ "$ready_val" -eq 0 ]; then
      log=$(log_for "$shard")
      # FIX: stdout-non-empty test, not exit code
    ...
    # Judge progress (only when ready and running)
    eval "ready_val=\$ready_$shard"
    if [ "$ready_val" -eq 1 ] && [ "$state" = "Running" ]; then
      ...
      if [ "$judge_val" -eq 0 ] && [ "$cur_lines" -gt 0 ] 2>/dev/null; then
        ...
        if [ "$cur_lines" -gt "$prev_lines" ] 2>/dev/null && [ $((now - prev_ts)) -ge 300 ]; then
          ...
          eval "lines_$shard=$cur_lines"
          eval "ts_$shard=$now"
        elif [ "$cur_lines" -eq "$prev_lines" ] 2>/dev/null && [ $((now - prev_ts)) -ge 1200 ]; then
          emit "[shard $shard] STALL: no new records in 20 min (lines=$cur_lines)"
          eval "ts_$shard=$now"
        fi
      fi
    fi
  ...
  # Exit only when BOTH shards have succeeded
  eval "s0=\$success_0"
  eval "s1=\$success_1"
  if [ "$s0" -eq 1 ] && [ "$s1" -eq 1 ]; then
    ...
    emit "To merge:  cat data/audit/v2_full_h100_shard{0,1}.jsonl > data/audit/v2_full_h100.jsonl"
    emit "Raw merge: cat data/audit/v2_full_h100_shard{0,1}.raw.jsonl > data/audit/v2_full_h100.raw.jsonl"
    exit 0
  fi
