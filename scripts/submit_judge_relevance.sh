...
WORKERS="${WORKERS:-8}"
MAX_TOKENS="${MAX_TOKENS:-0}"        # 0 = omit (vLLM uses max_model_len - input)
...
VLLM_LOG_SUFFIX="${VLLM_LOG_SUFFIX:-}"   # suffix for vLLM log path (use to avoid collisions in H200/H100 races)
...
for shard in $(seq 0 $((SHARD_COUNT - 1))); do
  ...
  runai submit "$JOB_NAME" \
    ...
    -e VLLM_LOG_SUFFIX="$VLLM_LOG_SUFFIX" \
