#!/usr/bin/env bash
# Run inside a Run:ai pod. Starts vLLM and runs the LLM chunk filter for one shard.
#
# Required env vars:
#   SHARD_INDEX   0-based shard index (0..SHARD_COUNT-1)
#   SHARD_COUNT   total number of shards (one per GPU)
#
# Optional env vars (all have defaults):
#   REPO_DIR, MODEL, MAX_MODEL_LEN, MAX_NUM_SEQS, GPU_MEMORY_UTILIZATION,
#   GDN_PREFILL_BACKEND, WORKERS, NUM_PREDICT, TEMPERATURE, SHUFFLE_SEED,
#   FILTER_INPUT
...
VLLM_LOG="logs/vllm_shard${SHARD_INDEX}.log"
vllm serve "$MODEL" \
  --host 0.0.0.0 \
  --port 8000 \
  ...
  > "$VLLM_LOG" 2>&1 &
VLLM_PID=$!
echo "$VLLM_PID" > "logs/vllm_shard${SHARD_INDEX}.pid"
...
OUTPUT="data/llm_filtered_chunks_shard${SHARD_INDEX}.jsonl"
RESULTS="data/llm_filter_results_shard${SHARD_INDEX}.jsonl"

echo "Starting LLM filter: shard ${SHARD_INDEX}/${SHARD_COUNT}, workers=${WORKERS}"
python3 scripts/llm_filter_chunks.py \
  --backend openai \
